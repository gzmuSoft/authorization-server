package cn.edu.gzmu.authserver.auth.email;
...
public interface EmailUserDetailsService {
    ...
    /**
     * 根据用户名找到用户。在实际实现中，只用查询手机号即可，对于手机号的验证需要对 {@link cn.edu.gzmu.authserver.validate.email} 进行具体公职
     * 如何找寻用户具体取决于实现实例的配置方式。
     * <p>
     * 在这种情况下，返回的<code> UserDetails </code> 对象的用户名是可能是不是用户 email 设备号。
     *
